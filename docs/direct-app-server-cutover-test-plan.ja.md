...
承認画面の実引数を元の会話へ安全に表示するUIと一般の追加入力フォームは未完了。依頼中の同種ツール許可は、構造化された実callと確認要求を照合できる操作に限る。未対応の確認では拒否か `/stop` を使う。テスト中の不具合は元投稿・Botの表示・時刻を記録し、同じ依頼を自動で繰り返さない。
...
| U-15 依頼中の同種ツール許可 | 同じ依頼でPlaywrightの対象ツールを2回呼び、依頼中許可を選ぶ。別の新規依頼でも1回呼ぶ | 1件目に「この依頼中、このツールを許可」「今回だけ許可」「拒否」が出る。2件目だけ再確認を省き、新規依頼では改めて確認する。引数変更を含む範囲が画面に明示される |
| U-16 長いコマンドの承認 | 1900文字を超えるコマンドで承認を発生させ、本人限定の「次へ」を最後まで開く | 実引数の全ページが表示され、最終ページにだけ許可が出る。途中でも上流が提示した拒否または取消を使え、古い画面からは許可できない |
...
| S-04a 依頼中許可の失効 | 同じツールで異なる引数、別ツール、証拠欠落・重複、Steer・停止・取消、Run終端、子プロセス再起動 | 同一Runの照合済み対象だけ適用し、入力世代変更や停止後の呼出しを自動承認しない。単発の選択から依頼中許可を作らない |
