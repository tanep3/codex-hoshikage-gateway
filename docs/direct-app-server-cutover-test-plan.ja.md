...
3. `/models`、`/model`を開き、一覧と選択値を確認する。モデル変更は一覧に出た別IDだけを指定する。
4. 「この会話の作業フォルダーに小さな report.txt を作り、内容を 'preview ok' にしてください」と依頼する。表示された操作を確認して本人が許可または拒否し、回答後に `/get path:report.txt` で保存版を確認する。
