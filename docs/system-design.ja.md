...
カタログ更新中は同じinteraction revisionで期限内の直前の正常な公開カード／本人向け入口を保持する。保持するのは画面表示だけで、許可は押下時の再取得・scope照合を必須とする。WAITING監視の60秒上限は維持し、更新が終わらなければ再確認案内へ移行する。再起動後は正常表示を再確認してから保持対象にする。
