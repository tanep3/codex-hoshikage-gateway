...
async fn approval_updates_in_place_and_typing_pauses_until_decision() {
    ...
    let router=Router::new()
      ...
      .route("/interactions/{id}/token/callback",post(||async{StatusCode::NO_CONTENT}))
      .route("/webhooks/9/token/messages/@original",axum::routing::patch(move||{let reply=reply.clone();async move{reply.fetch_add(1,Ordering::SeqCst);Json(json!({}))}}));
    ...
    tx.send(Incoming::Interaction(json!({"id":"200","guild_id":"1","channel_id":"4","application_id":"9","token":"token","member":{"user":{"id":"2"}},"data":{"custom_id":"approval:approval_a:accept"}}))).await.unwrap();
    tokio::time::timeout(Duration::from_secs(5), async {
        while replies.load(Ordering::SeqCst) == 0 {
    ...
    tx.send(Incoming::Interaction(json!({"id":"201","guild_id":"1","channel_id":"4","application_id":"9","token":"token","member":{"user":{"id":"2"}},"data":{"custom_id":"approval:approval_a:accept"}}))).await.unwrap();
    tokio::time::timeout(Duration::from_secs(5), async {
