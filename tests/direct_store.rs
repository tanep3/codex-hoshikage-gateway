...
fn schema_nine_upgrades_without_reinterpreting_proxy_records() {
    ...
    c.execute_batch("DROP TRIGGER direct_dispatch_no_rewind; DROP TABLE direct_answers; DROP TABLE direct_dispatches; DROP TABLE direct_conversations; DELETE FROM schema_migrations WHERE version=10; UPDATE schema_meta SET schema_version=9;").unwrap();
    drop(c);
